...
- Fork this repo by pressing the Fork button at the top of this webpage
     - When forking **UNCHECK** the default option to "Copy the main branch only". You want all the branches!
     -  Forking this repo made a copy of this repo on YOUR GitHub.
...
- Let's get a local copy of your forked repo...
     - login to Datahub
     - if you have not already created SSH keys for datahub, do so now and place them on your GitHub account per https://docs.github.com/en/authentication/connecting-to-github-with-ssh/adding-a-new-ssh-key-to-your-github-account  **NOTE: these keys are different than the ones you made of your laptop! Each computer you use needs its own keys**
     - Use the Datahub file browser to navigate to the `private` directory inside your home directory.  Wherever you are in the file browser is where the new git repo will be created.
     ...
     -  Click the button "Clone a Repository" and set remote URL to `git@github.com:your_username/GitExercise_Conflicts.git` where you must replace `your_username` with your actual GitHub username. Click the button that says `Clone`
...
## After Part I / II DO THIS or YOU WILL LOST POINTS 
